...
LR = 1e-5
BATCH_SIZE = 12
print('***************v1:2 robert_zh_large_epoch5_lr1_ml64_bs12***************')
...
def train(fold_all):
    config = BertConfig.from_pretrained('../../model_lib/robert/pytorch/RoBERTa_zh_Large_PyTorch/config.json')
    ...
    for fold_index in range(FOLD):
        ...
        print('正在加载模型...')
        if USE_GPU:
            model = BertForSequenceClassification.from_pretrained('../../model_lib/robert/pytorch/RoBERTa_zh_Large_PyTorch/', config=config).cuda()
            # model = BertForSequenceClassification.from_pretrained('../../model_lib/bert/pytorch/xs/', config=config).cuda()
        else:
            model = BertForSequenceClassification.from_pretrained('../../model_lib/robert/pytorch/RoBERTa_zh_Large_PyTorch/', config=config)
        ...
        for epoch in range(EPOCH):
            ...
            if f1_val > BEST_F1:
                BEST_F1 = f1_val
                BEST_EPOCH = epoch
                # 1080_7:2 large_wwm_epoch5_lr2_ml64_bs6_
                torch.save(model, 'robert_zh_large_epoch5_lr1_ml64_bs12_' + str(fold_index) + 'k_' + 'best_model.m')
...
def test(test_dataloader):
    ...
    for fold_index in range(FOLD):
        # 1080_7:2 large_wwm_epoch5_lr2_ml64_bs6_
        model = torch.load('robert_zh_large_epoch5_lr1_ml64_bs12_' + str(fold_index) + 'k_' + 'best_model.m')
        result_list = []
        result_logits_list = []
        model.eval()
